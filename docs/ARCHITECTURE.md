...
| `tests/` | 29 file Vitest (`grid`, `scoring`, `bombe`, `generator`, `engine`, `distribuzioni`, `quadri`, `i18n`, `sfide`, `documenti`, `invarianti`, `privacy`, `script`, `durate`, `contrasti`, `icona`, `android`, `anteprima`, `anteprima-collegamento`, `accoglienza`, `archivio`, `profilo`, `scala`, `scheda`, `incitamenti`, `sacchetto`, `trionfo`, `ripresa`, `salvataggio`) più 19 scenari in `e2e/`, uno per file, tutti registrati in `tools/verifica-tutto.mjs` | `core/`, `config/`, `i18n/`, `persistence/`, `styles/`, Playwright | Node |
...
- `npm test` passa: **499 test in 29 file**, ~20 s (i più lunghi sono `incitamenti.test.js`,
