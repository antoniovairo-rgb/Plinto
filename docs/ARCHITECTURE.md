...
| `src/ui/` | `App.jsx`, `SchermoGioco.jsx`, i componenti `Plancia`, `Tray`, `Pezzo`, `Hud`+`BarraCatena`, `Logo`, `Annunci`, gli hook `useTrascinamento` e `useTastiera`, e in `schermate/` le schermate (fra cui `Quadri`, `AperturaQuadro`, `FineQuadro`) più l'impalcatura comune `Pagina.jsx` | `core/`, `config/`, `state/`, `feel/`, `audio/`, `i18n/`, React | browser |
...
- `npm test` passa: **165 test in 14 file**, ~12.5 s (undici e mezzo dei quali spesi nel solo
...
- Il costo — perdere il controllo statico — è compensato in parte dai commenti `@param` /
  ...
  test, che sul `core/` resta la parte più densa: dei 165 test, 102 riguardano il `core/`
  (griglia, punteggio, bombe, generatore, motore, invarianti), 22 i Quadri (definizione, svolgimento e testi della schermata di apertura), 7 l'i18n, 6 la
