...
`npm test`: **522 test in 32 file, tutti verdi**, durata ~20 s (misurato il 15 settembre
...
| `tests/anteprima.test.js` | 11 | **il test che conta di tutta la fase**: la terna mostrata in anteprima è identica a quella poi consegnata — forma e bombe — su **oltre mille mani**, ed è lo *stesso oggetto*, quindi non c'è nemmeno lo spazio per ricalcolarla. Più: che l'anteprima non cambi fra una mossa e l'altra della stessa mano, che la modalità base non abbia perso una sola estrazione, che una partita salvata prima che le modalità esistessero si riprenda come base |
...
| `tests/e2e/sicurezza.mjs` | `npm run e2e-sicurezza` | la politica di sicurezza dei contenuti, provata **sulla build** e non sul server di sviluppo. Due metà: il gioco intero (home, sette schermate, una partita, il service worker, il salvataggio che si scarica con un indirizzo `blob:`) non deve produrre **nessuna** violazione; poi uno script, un'immagine e una chiamata di rete verso un altro dominio **devono** essere bloccati. Senza la seconda metà il controllo sarebbe vuoto: il giorno in cui il `<meta>` sparisse per sbaglio, «zero violazioni» sembrerebbe una buona notizia. Verificato capace di fallire: togliendo il plugin dalla build, segnala quattro problemi |
...
| `tools/prova-desktop.mjs` | `npm run prova-desktop` | apre il gioco su quattro schermi grandi (1631 × 1030, 1920 × 1200, 1440 × 700, 820 × 1180) e misura una cosa concreta e non opinabile: **quanti pixel di vuoto** restano fra la fine del contenuto della presentazione e il pulsante GIOCA. Oltre 180 px è un errore. Poi entra in partita e controlla che la plancia non sia più bassa di 240 px. Salva una schermata per ciascuno. Dalla 1.0.1 apre la home anche su **quattro formati di telefono** nello stato più affollato (partita e sfida entrambe in corso), e fallisce se la testata è tagliata, se la versione sparisce o se del contenuto resta fuori **senza poter scorrere**. Esiste perché è successo: la home è cresciuta di tre voci in tre versioni e su 360×800 il logo è finito sotto la barra di stato — l'ha visto un giocatore, non un test |
